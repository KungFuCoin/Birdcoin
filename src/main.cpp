...
uint256 hashGenesisBlock("0x974e6e3470cba91409d38190df1f6e35f0d7696ea82213d10f67d8e40d05fe05");
...
{
    // Birdcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60 * 60; // Birdcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Birdcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Birdcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Birdcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x974e6e3470cba91409d38190df1f6e35f0d7696ea82213d10f67d8e40d05fe05");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Happiness is a Kung Fu Sing, with good fortune let it ring. Ah the Kung Foo Sing";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x885206fce47cf0c3c9bf11fb133868c95ef9a445a333f267c608d70ecac36400"));
...
unsigned char pchMessageStart[4] = { 0xeb, 0xd0, 0xc6, 0xcb }; // Birdcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BirdcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BirdcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BirdcoinMiner(CWallet *pwallet)
{
    printf("BirdcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("birdcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BirdcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BirdcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BirdcoinMiner, pwallet));
}
